...
#: A release carries its own provenance, because an installed release has no
#: ``.git`` to ask.
RELEASE_MANIFEST_FILENAME = "release.manifest.json"

#: Deployment-owned environment layer read by the systemd unit. See
#: ``ReleaseStore.write_state_env``.
STATE_ENV_FILENAME = "service.env"
...
class ReleaseStore:
    ...
    def state_env_path(self) -> str:
        return os.path.join(self.state_dir, STATE_ENV_FILENAME)

    def write_state_env(self, release_id: str, source_git_sha: str) -> Dict[str, Any]:
        """Publish the active release's identity for the systemd unit to read.

        The node validates at startup that the code it is about to run is the
        commit it was told to expect. Under Phase 13F that expectation was a
        single pinned SHA in the root-owned ``/etc`` env file, which was correct
        when one git checkout ran forever.

        It breaks the moment releases exist. Every release carries a different
        ``source_git_sha``, so a single pinned value makes at most one release
        able to start; the rest fail preflight and get rolled back. Pinning it
        per activation would mean editing ``/etc`` -- and therefore sudo -- for
        every update, which would defeat the point of A/B deployment.

        So the unit layers a second ``EnvironmentFile`` from this file, which the
        deployment manager owns and can write without privilege, and which
        overrides the ``/etc`` default because systemd applies environment files
        in order.

        Being honest about what this is not: the file is writable by the service
        user, so it is not a defence against a compromised node -- that node
        already runs the code. It grants no privilege that was not already held.
        Integrity here rests on the package and engine SHA-256 values in the
        release manifest, which is exactly the guarantee this phase claims.
        Authenticity is Phase 13H.
        """

        os.makedirs(self.state_dir, exist_ok=True)
        body = (
            "# Generated by the Phase 13G deployment manager. Do not edit.\n"
            "# Overrides the pinned default in the root-owned /etc env file so a\n"
            "# release can be activated without sudo. Rewritten on every switch.\n"
            "MA_VLNA_EXPECTED_SHA=%s\n"
            "MA_VLNA_ACTIVE_RELEASE_ID=%s\n"
        ) % (source_git_sha, release_id)

        target = self.state_env_path()
        handle = tempfile.NamedTemporaryFile(
            mode="w", encoding="utf-8", dir=self.state_dir,
            prefix=".%s." % STATE_ENV_FILENAME, delete=False,
        )
        try:
            handle.write(body)
            handle.flush()
            ...
            handle.close()
        # systemd reads this at unit start; a half-written file would be parsed
        # as truth, so it is renamed into place rather than written in place.
        os.replace(handle.name, target)
        self._fsync_dir(self.state_dir)
        return {
            "path": target,
            "release_id": release_id,
            "expected_sha": source_git_sha,
            "atomic": True,
        }

    def release_source_sha(self, release_id: str) -> str:
        """``source_git_sha`` from the release's own manifest, or ``""``."""

        path = os.path.join(self.release_path(release_id), RELEASE_MANIFEST_FILENAME)
        try:
            with open(path, "r", encoding="utf-8") as handle:
                payload = json.load(handle)
        except (OSError, ValueError):
            return ""
        value = payload.get("source_git_sha")
