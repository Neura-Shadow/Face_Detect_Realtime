...
- **Phase 13**:
  ...
  - Phase 13D through 13F: **Passed** - INT8 calibration built, gated and deliberately frozen `experimental_non_authoritative` rather than promoted (Phase 13D-MP-RECOVERY; FP16 remains the sole command-authority precision), FP16 continuous soak, bounded clock-drift/backpressure/memory hardening (13E-R Full Pass), and a supervised Jetson `systemd` service with verified boot recovery (13F Boot Pass). See `docs/phase13d_int8_calibration_range_shift.md`, `docs/phase13d_mp_recovery.md`, `docs/phase13e_fp16_soak.md`, `docs/phase13er_clock_drift_backpressure_recovery.md`, `docs/phase13f_jetson_service_supervision.md`.
  - Phase 13G-OTA-A-B-ROLLBACK-VERSION-COMPATIBILITY: **Pass - Gates A through E complete on the physical Jetson Orin NX.** Application-level A/B release mechanism: atomic symlink activation (never observably absent under concurrent switching), automatic rollback on failed readiness or probation, manual rollback, and boot-interrupted-update recovery verified across two real reboot cycles, each proven by a differing kernel boot ID rather than by state inspection alone. A genuine `systemd` unit-ordering deadlock in the boot-recovery path was found and fixed by that testing. 670 tests, 0 skipped, 16/16 mandatory scenarios covered. Provides SHA-256 integrity and version compatibility only — explicitly not Secure Boot, signed artifacts, bootloader A/B, or anti-rollback security. See [docs/phase13g_ota_ab_rollback.md](docs/phase13g_ota_ab_rollback.md).
...
Phase 13 begins embedded safety-contract verification. Phase 13A is host SIL only; it does not claim a real MCU, HIL, actuator, model-accuracy, OTA/security, or benchmark pass. See [docs/phase13a_embedded_contract_sil.md](docs/phase13a_embedded_contract_sil.md).
  ...
  - Phase 13C-TENSORRT-FP16-EDGE-PERCEPTION: **Pass - real Jetson Orin NX ran a target-built TensorRT FP16 YOLOv9-c engine in the command-authority path of the CARLA closed loop. Gate B engine built on target (52.8 MB, 50 warm-ups + 300 measured inferences, GR3D 96%, 0 CUDA errors, 0 per-frame device allocations); Gate C streamed 300/300 frames with 300 commands accepted and 0 rejected; Gate D ran 1200 Town03 ticks producing 300 camera frames, 304 inferences, 1197 active controls and 18 SAFE_STOPs with 0 command timeouts and `frame_to_command_ms_p99=99.1` against a 949.6 ms budget. Backend parity against the official YOLOv9 source: matched rate 1.000, class agreement 1.000, box IoU mean 0.954 (conversion consistency only, not accuracy). Fault matrix 28/28, `false_accept_count=0`, `false_reject_count=0`**
...
Phase 13C replaces the Phase 13B dummy backend with real TensorRT FP16 inference on the Jetson GPU, keeping the verified Phase 13B transport and safety path unchanged: the Phase 13A 64-byte command, the JILF 56-byte header, the JILA 48-byte ACK, mailbox depth 1 and the C Virtual Safety MCU authority are all untouched, and the Jetson node still defaults to `--perception-backend dummy`. An `AI_ACTIVE` command requires a fresh, no-fallback TensorRT result for the same frame id; anything missing, stale, timed-out or range-invalid yields SAFE_STOP. Precision is FP16 only; INT8 and QAT belong to Phase 13D. It does not claim model accuracy, mAP, perception quality, navigation quality or route completion. See [docs/phase13c_tensorrt_fp16_edge_perception.md](docs/phase13c_tensorrt_fp16_edge_perception.md).

Phase 13G closes the current engineering cycle: a commit becomes an immutable release, activation is atomic, an activation that cannot prove itself is rolled back automatically, and an update interrupted by power loss resolves deterministically on the next boot. It does not claim Secure Boot, signed OTA, bootloader/rootfs A/B, anti-rollback security, full HIL, real MCU/CAN/UART, physical camera/actuator, model accuracy, or physical deployment. For the complete Phase 1–13G status, the simulation-vs-real-hardware breakdown, and the explicitly unsupported claims, see [docs/final_project_status.md](docs/final_project_status.md); for operational commands, see [docs/final_runbook.md](docs/final_runbook.md).
...
Phase 12C-R1-RT-DETR-ASSET-BLOCKER-FREEZE latest summary:
