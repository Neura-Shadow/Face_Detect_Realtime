...
Phase 12A-C applied that matrix in a real CARLA runtime confirmation. All five calibrated routes reached the goal, with `confirmed_route_count=5` and `collision_count=0` for every route. This confirms the calibrated smoke setup while preserving the original 2500-step Phase 12A result as blocked.
