...
* **Phase 11: CARLA Closed-Loop Integration**
  ...
  - 新增 Phase 12C-YOLOv9-R1 Selected YOLOv9 Runtime Confirmation，將 YOLOv9 no-fallback readiness 後的第一個 selected runtime row 固定為 `route_01 + grp_follower + yolov9`。正式 evidence `experiments\phase12\20260630T094645Z` 顯示 `source_adapter_verified=true`、`edge_yolov9_fallback_used=false`、`edge_yolov9_no_fallback_verified=true`，但 `blocked_reason=CARLA server is not reachable`，因此 `runtime_confirmation_executed=false`、`carla_route_runtime_executed=false`。此為 blocked runtime attempt，不代表 full Phase 12C perception ablation runtime pass、YOLOv9 accuracy、RT-DETR runtime、Leaderboard、formal route benchmark 或 infraction benchmark。
