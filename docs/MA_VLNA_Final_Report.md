...
* **Phase 11: CARLA Closed-Loop Integration**
  ...
  - 新增 Phase 12C Perception Backend Ablation Prepared，固定 Phase 12B-SUM 選出的 `grp_follower` controller，建立 5 條 calibrated routes x `dummy` / YOLOv9 optional / RT-DETR optional 的 perception backend scaffold。此階段只做 dependency preflight 與 runtime command wiring；缺少 YOLOv9 optional dependency、EdgePerception 尚未支援 YOLOv9 backend、或缺少 RT-DETR dependency 時，optional rows 會標記為 `backend_unavailable`，不阻塞 dummy baseline，也不宣稱 YOLOv9 / RT-DETR runtime pass。
  ...
  - 新增 Phase 12C-YOLOv9-U YOLOv9 Optional Dependency Unlock Prepared，將早期 generic Phase 12C-YOLO-U 修訂為 YOLOv9-specific target；針對 dedicated CARLA Python 3.12 runtime 檢查 YOLOv9 import、pip metadata 與 `EdgePerception --test yolov9` 支援性，確認目前 `edge_yolov9_command_supported=false`，並產生 operator-driven manual unlock commands 與 post-unlock verification commands。此階段不自動安裝、不修改 baseline requirements、不啟動 CARLA，也不宣稱 YOLOv9 runtime pass。
