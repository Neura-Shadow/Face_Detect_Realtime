...
* **Phase 11: CARLA Closed-Loop Integration**
  ...
  - 新增 Phase 12C-YOLOv9-V YOLOv9 Post-Unlock Verification，建立 strict verifier 檢查 CARLA Python 3.12 import、pip metadata、EdgePerception no-fallback、Phase 12B/11M/11K/baseline mapper `yolov9` CLI wiring 與 Phase 12C rows refresh。目前本機仍缺 YOLOv9 dependency，因此 `post_unlock_verified=false`，且 `--require-verified` strict gate 實測 exit code 1；此為 blocked evidence，不自動安裝套件，也不宣稱 YOLOv9 runtime pass。
