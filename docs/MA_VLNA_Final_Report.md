...
* **Phase 11: CARLA Closed-Loop Integration**
  ...
  - 新增 Phase 12C Perception Backend Ablation Prepared，固定 Phase 12B-SUM 選出的 `grp_follower` controller，建立 5 條 calibrated routes x `dummy` / YOLOv9 optional / RT-DETR optional 的 perception backend scaffold。此階段只做 dependency preflight 與 runtime command wiring；缺少 YOLOv9 optional dependency 或缺少 RT-DETR dependency 時，optional rows 會標記為 `backend_unavailable`，不阻塞 dummy baseline，也不宣稱 YOLOv9 / RT-DETR runtime pass。
