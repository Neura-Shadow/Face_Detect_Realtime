...
* **Phase 11: CARLA Closed-Loop Integration**
  ...
  - 新增 Phase 12C Perception Backend Ablation Prepared，固定 Phase 12B-SUM 選出的 `grp_follower` controller，建立 5 條 calibrated routes x `dummy` / YOLOv9 optional / RT-DETR optional 的 perception backend scaffold。此階段只做 target CARLA Python dependency preflight 與 runtime command wiring；缺少 YOLOv9 optional dependency 或缺少 RT-DETR dependency 時，optional rows 會標記為 `backend_unavailable`，不阻塞 dummy baseline，也不宣稱 YOLOv9 / RT-DETR runtime pass。
  ...
  - 新增 Phase 12C-YOLOv9-B EdgePerception YOLOv9 Backend Adapter Prepared，為 `workers.core.edge_perception` 加入 `YOLOv9PerceptionBackend`、`backend="yolov9"` factory branch 與 CLI `--test yolov9`。base Python 與 CARLA Python 3.12 均已通過 adapter smoke；因 YOLOv9 optional dependency 尚未安裝，結果仍使用 fallback，僅代表 adapter path prepared，不代表 YOLOv9 runtime inference pass。
  - 新增 Phase 12C-YOLOv9-V YOLOv9 Post-Unlock Verification，建立 strict verifier 檢查 CARLA Python 3.12 import、pip metadata、EdgePerception no-fallback、Phase 12B/11M/11K/baseline mapper `yolov9` CLI wiring 與 Phase 12C rows refresh。目前權威 evidence `experiments\phase12\20260629T125701Z` 顯示本機仍缺可安裝/可匯入的 YOLOv9 dependency，因此 `post_unlock_verified=false`，且 `--require-verified` strict gate 實測 exit code 1；此為 blocked evidence，不自動安裝套件，也不宣稱 YOLOv9 runtime pass。
