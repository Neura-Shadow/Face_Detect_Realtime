...
- [x] **Phase 13C-TENSORRT-FP16-EDGE-PERCEPTION Prepared**: Gate A passed (100/100 Phase 13C unit tests, Python 3.8 parse gate, Phase 13B unit regression). Gates B/C/D are **blocked** by `onnx_export_dependency_missing`.
- [x] **Blocker is verified, not assumed**: all three PC environments with PyTorch (`ma-vlna-carla312` 3.12.13/torch 2.12.1, `anaconda3` 3.10.14/torch 2.12.0, `test_env` 3.10.14/torch 2.12.0) lack the `onnx` package, and torch 2.12 raises `OnnxExporterError('Module onnx is not installed!')` on every export path including `dynamo=False`. The Jetson has neither torch nor onnx. Phase 13C never installs dependencies automatically; the exact operator unlock command is emitted instead.
- [x] **External assets verified present and uncommitted**: YOLOv9 source `D:\AIModels\yolov9` (git repo with `export.py`, `models/`, `utils/`) and weights `yolov9-c-converted.pt`, 51 477 927 bytes, SHA-256 `39b6b490b1e4034b4fefdcdac81380155e0a30ba6c938a9751e2c351f81a517f`. No model asset is committed, downloaded or auto-installed.
- [x] **Export path does not auto-install**: the external `export.py` calls `check_requirements('onnx')`, which pip-installs. Phase 13C drives `torch.onnx.export` directly through the repository's existing trusted YOLOv9 loader, reproducing that branch's contract (`images` -> `output0`, constant folding, fixed `1x3x640x640`, opset 12) and deriving the output contract from a real reference forward pass rather than guessing.
- [x] **Engine is bound to its target**: the engine cache key hashes ONNX SHA-256, TensorRT version, CUDA version, GPU, compute capability, precision, input profile and postprocess profile; any mismatch, a tampered engine file, or a manifest without `built_on_target` marks it stale and forces a rebuild on the Jetson.
- [x] **CUDA allocator is honest about the board**: PyCUDA and cuda-python are absent from the Jetson venv and must not be installed, so the runtime drives `libcudart` through a narrow ctypes wrapper with every call status-checked, one reusable stream, buffers allocated once, and `per_frame_device_allocation_count = 0` required.
- [x] **TensorRT 8 API only**: `num_bindings` / `binding_is_input` / `get_binding_shape` / `set_binding_shape` / `execute_async_v2`; a non-8 major version is refused. `trtexec` flag selection reads the binary's own `--help` instead of assuming TensorRT 10 flags.
- [x] **Range scope stays honest**: `input_range_checked=true`, `tensor_output_range_checked=true`, `detection_schema_checked=true`, `activation_range_checked=false`, `quantization_saturation_checked=false`, `range_validation_scope=tensorrt_fp16_input_and_final_output`. A serialized plan exposes no internal activations, so none are claimed.
- [x] **Fault matrix**: 22 TensorRT cases (T01..T22) all pass at Gate A with `false_accept_count=0` and `false_reject_count=0`; the invariant asserted throughout is that no invalid TensorRT result may ever produce `AI_ACTIVE`.
- [x] **Phase 13B is not regressed**: the Jetson node's `--perception-backend` defaults to `dummy`, the loopback gate still reports Prepared with fault matrix 28/28, Phase 13A SIL still 28/28, portable C CTest still 2/2, Phase 11 checks 10/10 (Phase 13C added two checks to `run_phase11_carla_checks.py`, so the suite grew from 8 to 10; the actual current count is 10/10) and demo checks 6/6.
- [x] **Boundary preserved**: `precision=fp16`, `int8_engine_built=false`, `int8_calibration_verified=false`, `qat_verified=false`, `model_accuracy_verified=false`. No mAP, perception quality, navigation quality, route completion, full HIL, real MCU, physical camera/actuator, Leaderboard or infraction claim. Model assets, engines, calibration caches and raw runtime logs are never committed.
